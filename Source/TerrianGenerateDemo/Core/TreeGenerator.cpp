...
void TreeGenerator::GenerateTree(Chunk& chunk){
	...
	for (int j = 0; j < MaxBlocksWidth; ++j)
	{
		...
		float possible = (NoiseTool::hash21(chunk.ChunkPosition*11 + pf*1024))/1024.0f - FMath::Abs(valueTemperature+0.1f)*0.15f;

        //若满足概率
        if(possible > 0.9f){
            valueTemperature += ((NoiseTool::hash21(chunk.ChunkPosition*17 + pf*1024))-512.0f)/512.0f*0.1f;
		...
		chunk.BlocksTemperature[i][j] = valueTemperature;
