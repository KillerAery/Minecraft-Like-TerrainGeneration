		...
		for (int i = 0; i < MaxBlocksWidth; ++i)
			for (int j = 0; j < MaxBlocksWidth; ++j)
			{
				...
				BlocksHeight[i][j] =
					...
					(FMath::Clamp<float>(
						FMath::Lerp(
							FMath::Lerp(NoiseTool::grad(vertexAfterHash22[0].GetSafeNormal(), pf),
							NoiseTool::grad(vertexAfterHash22[1].GetSafeNormal(), pf - FVector2D(1.0f, 0.0f)),
							w.X),
							FMath::Lerp(NoiseTool::grad(vertexAfterHash22[2].GetSafeNormal(), pf - FVector2D(0.0f, 1.0f)),
							NoiseTool::grad(vertexAfterHash22[3].GetSafeNormal(), pf - FVector2D(1.0f, 1.0f)),
							w.X),
						w.Y), 0.0f, 3.0f) * 0.5f * m);
					*/
