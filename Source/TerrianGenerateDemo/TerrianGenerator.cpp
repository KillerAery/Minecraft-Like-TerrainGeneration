		...
		for (int j = 0; j < MaxBlocksWidth; ++j)
		{
			...
			BlocksHeight[i][j] =
				...
				0.5f * (FMath::Clamp<float>(FMath::Lerp(
					FMath::Lerp(NoiseTool::grad(NoiseTool::hash22(vertex[0]), pf),
						NoiseTool::grad(NoiseTool::hash22(vertex[1]), pf - FVector2D(1, 0)),
						w.X),
					FMath::Lerp(NoiseTool::grad(NoiseTool::hash22(vertex[2]), pf - FVector2D(0, 1)),
						NoiseTool::grad(NoiseTool::hash22(vertex[3]), pf - FVector2D(1, 1)),
						w.X),
					w.Y), -1.0f, 1.0f) * 5 + 20);
